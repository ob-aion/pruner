...
- **README rewrite** — the front door now leads with the four-month threat surface (ClawHavoc + ToxicSkills + Cato CTRL MedusaLocker), explains the wrapper rationale (Cisco does the engine work; Pruner adds the trust artefact + Coroboros pack), states the trust-artefact spec vision (`report-v1` + SBOM + in-toto attestations as a candidate OpenSSF WG-Supply-Chain-Integrity contribution at 1.0), and ships an honest comparison table against `cisco-ai-defense/skill-scanner`, `snyk/agent-scan`, `affaan-m/agentshield`, and the skills.sh install-time audit.
- **AgentShield citation** added to `CLAUDE.md` anti-scope, `docs/threat-model.md` out-of-scope, `docs/coverage-matrix.md` MCP-runtime row. Different trust boundary (consumer-side runtime vs. publisher-side static); complementary, not competitive. Pruner answers *"is this skill safe to ship?"*; AgentShield answers *"is this agent setup safe to load?"*

...
def test_emit_shape() -> None:
    sarif = findings_to_sarif([_sample()], tool_name="pruner-wrapper", tool_version="0.2.0")
...
def test_write_and_read(tmp_path: Path) -> None:
    ...
    write_sarif([_sample()], out, tool_name="pruner-wrapper", tool_version="0.2.0")
    sarif = read_sarif(out)
    assert sarif["version"] == SARIF_VERSION


def test_empty_findings_emit_empty_results() -> None:
    sarif = findings_to_sarif([], tool_name="pruner-wrapper", tool_version="0.2.0")
